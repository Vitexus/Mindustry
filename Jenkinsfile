...
// Mindustry requires Java 17 to build (settings.gradle hard-fails otherwise).
// Confirmed via `docker run <dist> apt-cache show openjdk-17-jdk`:
// debian:trixie/forky and ubuntu:oracular dropped openjdk-17-jdk (21/25/26/27
// only) - only list distributions here that still ship it.
            ...
            ansiColor('xterm') {
                ...
                stage('Build ' + distroName) {
                    buildImage.inside {
                        ...
                        sh 'sudo apt-get install -y openjdk-17-jdk-headless'
